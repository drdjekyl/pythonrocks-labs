...
def fig_decennies(df):
    ...
    ax.plot(g.index, g["median"], color=SERIES[0], linewidth=2, marker="o", markersize=5,
            markeredgecolor=SURFACE, markeredgewidth=2, label="Médiane de la flotte")

    # Étiquettes directes en bout de courbe : obligatoires, l'orange passe sous 3:1.
    for serie, couleur, texte in [("max", SERIES[1], "Le plus grand"),
                                  ("median", SERIES[0], "Médiane")]:
        ax.annotate(f"{texte}\n{g[serie].iloc[-1]:.0f} m",
                    xy=(g.index[-1], g[serie].iloc[-1]),
                    xytext=(10, 0), textcoords="offset points",
                    color=couleur, fontsize=10, fontweight="600", va="center")
    ...
    titrer(ax, "La flotte ne grandit pas — son sommet s'étire",
...
def fig_loi_echelle(df):
    ...
    ax.plot(xs, np.exp(ordonnee) * xs**pente, color=ACCENT, linewidth=2,
    ...
    ax.plot(xs, k * xs**3, color=CONTEXTE, linewidth=2, linestyle="--",
    ...
    titrer(ax, "Un yacht deux fois plus long n'est pas huit fois plus gros",
           f"{n:,}".replace(",", " ")
...
def fig_piege_v3(df):
    ...
    print(f"    (pente globale {pente_log(d, 'max_speed', 'main_eng_power')[0]:.2f}, "
          f"n={len(d)})")
    ...
    for ax, (bas, haut, nom), couleur in zip(axes, bandes, SERIES, strict=True):
        ...
        ax.scatter(sous.max_speed, sous.main_eng_power, s=9, alpha=0.28, color=couleur,
                   linewidths=0, rasterized=True)
        ...
        ax.text(0, 1.005, f"pente {fr(pente)} · n={espace(n)}", transform=ax.transAxes,
                fontsize=10, color=couleur, fontweight="600", va="bottom")
...
def fig_chantiers(df):
    ...
    titrer(ax, f"Un marché fragmenté : {espace(len(b))} chantiers, aucun dominant",
...
def fig_biais(df):
    ...
    ax.annotate("Aucun navire sous 26 m", xy=(26, ax.get_ylim()[1] * 0.82),
                xytext=(34, ax.get_ylim()[1] * 0.82), fontsize=10,
                color=SERIES[1], fontweight="600", va="center",
                arrowprops={"arrowstyle": "->", "color": SERIES[1], "linewidth": 1.4})
    au_dela = int((df.overall_length > 120).sum())
    ax.set_xlim(0, 120)
    ax.text(0.985, 0.62, f"+ {au_dela} navires\nau-delà de 120 m", transform=ax.transAxes,
            ha="right", fontsize=10, color=MUTED)
    ax.set_xlabel("Longueur hors-tout (m)")
    ax.set_ylabel("Nombre de navires")
    ax.grid(axis="x", visible=False)
    titrer(ax, "Ce catalogue n'est pas un recensement",
